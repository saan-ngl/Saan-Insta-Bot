...
class MessageQueue extends require('events').EventEmitter {
        constructor(options = {}) {
                ...
                // Cap pending retry timers to prevent unbounded accumulation
                this.pendingRetries = 0;
                this.maxPendingRetries = 50;
        ...
        async _processMessage(message) {
                try {
                        ...
                        if (message.threadID && this._isThreadRateLimited(message.threadID)) {
                                // Requeue with delay — capped to prevent timer accumulation
                                if (this.pendingRetries < this.maxPendingRetries) {
                                        this.pendingRetries++;
                                        setTimeout(() => {
                                                this.pendingRetries--;
                                                this.enqueue(message, 5);
                                        }, this.threadWindow);
                                }
                                return;
                        }
